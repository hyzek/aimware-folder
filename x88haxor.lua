...
function oMenu.addBoolSwitch(name, i, guiObject, default)
    ...
    if gui.GetValue(oMenu["VARS"][i]) then
        ...
        draw.Text(x+110, y, "ON")
    else
        draw.Color(255, 255, 255, 255)
        draw.Text(x+110, y, "OFF")
    end
...
function oMenu.addDropDown(name, i, guiObject, rangeMax, drawName)
    ...
    if gui.GetValue(oMenu["VARS"][i]) > 0 then
        draw.Color(31, 142, 255, 255)
        draw.Text(x+110, y, drawName[gui.GetValue( oMenu["VARS"][i] )])
    else
        draw.Color(255, 255, 255, 255)
        draw.Text(x+110, y, "OFF")
    end
...
function oMenu.addAAMode(name, i, guiObject, rangeMax, drawName)
    ...
    if(gui.GetValue(oMenu["VARS"][i]) == 0) then
        draw.Color(31, 142, 255, 255)
        draw.Text(x+110, y, "Lowerbody")
    else
        draw.Color(31, 142, 255, 255)
        draw.Text(x+110, y, "Micromovement")
    end
...
function oMenu.addSlider(name, value, guiObject, default, minValue, maxValue, pValue)
    ...
    draw.Color(r, g,b)
    draw.Text(x+110, y, tostring(gui.GetValue(oMenu["VARS"][value])))
...
local function welcome()
    draw.SetFont( font1 );
    draw.Text(300, 7, "Hello Masterlooser :)");
    draw.Text(300, 22, "Hello Squidoodle :)"); --insert your retarded name here 

    draw.Text(300, 37, "Real: " .. math.floor(real * 100) / 100)
    draw.Text(410, 37, "LBY: " .. math.floor(lby * 100) / 100)
    draw.Text(500, 37, "Diff: " .. math.floor((real - lby) * 100) / 100)
    draw.Text(500, 150, "Kills: " .. kills)
    draw.Text(520, 165, "Deaths: " .. deaths)
    draw.Color(85 * kd, 100 * kd, 0 * kd, 255)
    draw.Text(500, 180, "KD: " .. math.floor(kd * 100) / 100)
    draw.Color(255, 255, 255, 255)
    draw.Text(500, 195, "Ping: " .. ping)
end
...
local function on_draw()
    ...
    local str_min_damage = "rbot.accuracy.weapon.".. handle_weapon(local_weapon) ..".mindmg"
